<aside id="mobile-nav">
    <nav>
        <ul>
            <li class="side-categories"><i class="fas fa-times-circle"></i></li>
            <li class="side-categories">
                <a href="/">Home</a>
            </li>
            {{-- <li class="side-categories">
                <a href="/tags/reviews">Reviews</a>
            </li>
            <li class="side-categories">
                <a href="/tags/blog">Blog</a>
            </li> --}}
            <li class="side-categories">
                <a href="">Search By Price</a>
            </li>
                <ul>
                    <li class="side-child-categories">
                        <a href="/category/budget/1000">$1000+</a>
                    </li>
                    <li class="side-child-categories">
                        <a href="/category/budget/500-1000">$500-1000</a>
                    </li>
                    <li class="side-child-categories">
                        <a href="/category/budget/401-500">$401-500</a>
                    </li>
                    <li class="side-child-categories">
                        <a href="/category/budget/301-400">$301-400</a>
                    </li>
                    <li class="side-child-categories">
                        <a href="/category/budget/201-300">$201-300</a>
                    </li>
                    <li class="side-child-categories">
                        <a href="/category/budget/101-200">$101-200</a>
                    </li>
                    <li class="side-child-categories">
                        <a href="/category/budget/100">Up to $100</a>
                    </li>
                </ul>

            <li class="side-categories">
                <a href="/category/headphones">Headphones</a>
            </li>
            <ul>
                <li class="side-child-categories"><a href="/category/headphones/fullsize">Full Size</a></li>
                <li class="side-child-categories"><a href="/category/headphones/portables">Portable</a></li>
                <li class="side-child-categories"><a href="/category/headphones/planars">Planar</a></li>
                <li class="side-child-categories"><a href="/category/headphones/electrostatics">Electrostatic</a></li>
            </ul>
            <li class="side-categories">
                <a href="/category/in-ears">Sources</a>
            </li>
            <ul>
                <li class="side-child-categories"><a href="/category/in-ears/custom-monitors">Custom Monitors</a></li>
                <li class="side-child-categories"><a href="/category/in-ears/balanced-armature-iems">Balanced Armature</a></li>
                <li class="side-child-categories"><a href="/category/in-ears/dynamic-iems">Dynamic</a></li>
                <li class="side-child-categories"><a href="/category/in-ears/hybrid-universal-iems">Hybrid Universal</a></li>
                <li class="side-child-categories"><a href="/category/in-ears/planar-magnetic-iems">Planar Magnetic</a></li>
                <li class="side-child-categories"><a href="/category/in-ears/wireless-iems">Wireless</a></li>
            </ul>
            <li class="side-categories">
                <a href="/category/source">Sources</a>
            </li>
            <ul>
                <li class="side-child-categories"><a href="/category/source/portable-source">DAPs</a></li>
                <li class="side-child-categories"><a href="/category/source/desktop-source">DACs</a></li>
            </ul>
            <li class="side-categories">
                <a href="/category/amplifiers">Amplification</a>
            </li>
            <ul>
                <li class="side-child-categories"><a href="/category/amplifiers/portable-amplifiers">Portable Amps</a></li>
                <li class="side-child-categories"><a href="/category/amplifiers/solid-state">Solid State</a></li>
                <li class="side-child-categories"><a href="/category/amplifiers/tube-amplifiers">Tubes</a></li>
            </ul>
            <li class="side-categories">
                <a href="/tags/best-gear">Best Gear</a>
            </li>
            <li class="side-categories">
                <a href="{{ route('contact') }}">Contact</a>
            </li>
            <li class="side-categories">
                <a href="{{ route('about') }}">About Us</a>
            </li>
            <li class="side-categories">
                <a href="{{ route('privacy') }}">Privacy Policy</a>
            </li>
            <li class="side-categories">
                <a href="{{ route('disclaimer') }}">Disclaimer/Copyright</a>
            </li>
        </ul>
    </nav>
</aside>
